...
typeCheckDefinition :: ModuleName -> FQDefinition -> TypeChecker Definition
typeCheckDefinition mname fqdef =
    withCache typedDefinitions (mname, unFQDefinition fqdef ^. defName)
              (go fqdef)
  where
    go :: FQDefinition -> TypeChecker Definition
    go = \case
...
typeCheck ctx fqterm = case fqterm of
  Variable i -> (Variable i,) <$> getVariable ctx i

  Reference (Ref mname name fqdef) -> do
    def@(Definition d) <- typeCheckDefinition mname fqdef
...
typeCheckPatternMatch ctx te pairs = do
    ...
    unless (checkNoneMissing missing) $
      throwError "non-exhaustive pattern matching"
...
typeCheckPattern = \case
  ...
  ConstructorPattern (Ref mname name def) cname patterns -> do
    ...
    subs <- forM (zip argTypes patterns) $ \(argType, pat) -> do
      let argMetaType = substDeBruijnIndex 0 (zip [0..] varMetaTypes)
...
substDeBruijnIndex base repl = \case
  ...
  FunctionTypeM t t' -> FunctionTypeM (substDeBruijnIndex base repl t)
...
checkNoneMissing :: MissingPatterns -> Bool
checkNoneMissing = \case
  MissingConstructors [] -> True
  _ -> False

removeMissingPattern :: MissingPatterns -> Pattern -> Maybe MissingPatterns
removeMissingPattern missing pat = case (missing, pat) of
  (MissingConstructors [], _) -> Nothing
  (_, WildcardPattern) -> Just $ MissingConstructors []
  (_, VariablePattern _) -> Just $ MissingConstructors []
  ...
  (MissingConstructors constrs, ConstructorPattern _ cname patterns) -> do
    ...
    if all checkNoneMissing missings'
      then Just $ MissingConstructors others
      else if changed
...
constructorType :: MonadError T.Text m => ModuleName -> TypeDefinition Type Kind
                -> T.Text -> m Type
